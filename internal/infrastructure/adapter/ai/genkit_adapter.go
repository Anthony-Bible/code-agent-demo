...
func (a *GenkitAdapter) SendMessageStreaming(
	...
	for value, iterErr := range genkit.GenerateStream(ctx, a.g, genOpts...) {
		...
		for _, p := range value.Chunk.Content {
			if cbErr := dispatchStreamPart(p, textCallback, thinkingCallback, thinkingEnabled); cbErr != nil {
...
// dispatchStreamPart routes a single streaming Part to the appropriate
// caller-provided callback. Text parts go to textCallback; reasoning parts
// go to thinkingCallback (only when thinking is enabled). Empty-text parts
// and nil callbacks are no-ops. Callback errors are wrapped with context
// describing which channel failed.
func dispatchStreamPart(
	p *genkitai.Part,
	textCallback port.StreamCallback,
	thinkingCallback port.ThinkingCallback,
	...
	case p.IsText():
		...
		if err := textCallback(p.Text); err != nil {
			return fmt.Errorf("text stream callback error: %w", err)
		}
	case p.IsReasoning():
		if thinkingCallback == nil || !thinkingEnabled || p.Text == "" {
			return nil
		}
		if err := thinkingCallback(p.Text); err != nil {
			return fmt.Errorf("thinking stream callback error: %w", err)
		}
	}
	return nil
}
...
func (a *GenkitAdapter) buildGenerateOptions(
	...
	if systemPrompt != "" {
		genOpts = append(genOpts, genkitai.WithSystem(systemPrompt))
	}
...
func (a *GenkitAdapter) registerTools(tools []port.ToolParam) ([]genkitai.ToolRef, error) {
	...
	// Hold the write lock across the entire registration sequence (lookup +
	// collision check + DefineTool + map insert) so concurrent first
	// registrations on shared clones can't race into a duplicate DefineTool
	// panic or a name-map overwrite.
	a.nameMapMu.Lock()
	defer a.nameMapMu.Unlock()
	...
	for _, t := range tools {
		...
		if existingOrig, alreadyRegistered := a.toolNameMap[sanitized]; alreadyRegistered {
			if existingOrig != t.Name {
				// Two distinct port names sanitize to the same genkit name.
				// Silently overwriting would corrupt the reverse mapping and
				// route the AI's tool call to the wrong implementation.
				return nil, fmt.Errorf(
					"tool name collision: %q and %q both sanitize to %q; "+
						"rename one to disambiguate",
					existingOrig, t.Name, sanitized,
				)
			}
			...
			// Mapping was recorded but the registry lost the entry (e.g. a
			// stubbed test registry). Fall through to re-register.
		}
...
func (a *GenkitAdapter) convertMessageToGenkit(msg port.MessageParam) *genkitai.Message {
	...
	case entity.RoleSystem:
		// System messages are passed via ai.WithSystem in buildGenerateOptions,
		// not inline. Forward as RoleSystem so genkit can route correctly if it
		// ever reaches this path; log to surface misuse to callers.
		a.logger.Warn("RoleSystem message in conversation slice; prefer opts.SystemPrompt")
		role = genkitai.RoleSystem
