...
RUN apk add --no-cache bash curl coreutils netcat-openbsd prometheus-node-exporter
...
find deploy/e2e/agent-investigations -type f -name '*.json'

# Read the investigation result
cat deploy/e2e/agent-investigations/investigations/*.json | jq .
...
while [ $ELAPSED -lt $TIMEOUT ]; do
  FILES=$(find agent-investigations -type f -name "*.json" 2>/dev/null | wc -l | tr -d ' ')
  if [ "$FILES" -gt 0 ]; then
    echo ""
    echo "=== Investigation complete! ==="
    echo ""
    find agent-investigations -type f -name "*.json" -print0 | while IFS= read -r -d '' f; do
